    ...
    fetch('http://127.0.0.1:8000/api/etkinlikler')
      ...
      .then(data => {
        const formattedData = data.map(dbEvent => ({
          id: dbEvent.id,
          title: dbEvent.title,
          location_text: dbEvent.location,
          coordinates: dbEvent.coordinates,
          category: dbEvent.type,
          date_start: dbEvent.date,
          deadline: "Yakında",
          level: "Genel",
        ...
        return (
          <div className="h-full overflow-y-auto pb-32 bg-[#f4f6f8] dark:bg-[#1C1018] px-6 pt-12 animate-fade-in transition-colors duration-500">
            ...
            <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide mb-4">
              <button onClick={() => setHomeFilter('all')} className={`px-5 py-2 text-xs font-bold rounded-full whitespace-nowrap transition-all ${homeFilter === 'all' ? 'bg-[#094D92] text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700'}`}>🔥 Sizin İçin</button>
              <button onClick={() => setHomeFilter('Hackathon')} className={`px-5 py-2 text-xs font-bold rounded-full whitespace-nowrap transition-all ${homeFilter === 'Hackathon' ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700'}`}>🚀 Hackathonlar</button>
                  ...
                  <div key={app.id} className="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <select value={app.type} onChange={(e) => updateApplicationStatus(app.id, e.target.value)} className={`appearance-none text-[10px] font-extrabold uppercase px-3 py-1.5 rounded-md cursor-pointer outline-none text-center shadow-sm border ${app.type === 'pending' ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 border-amber-100 dark:border-amber-800' : ''} ${app.type === 'success' ? 'bg-[#68B684]/10 text-[#68B684] border-[#68B684]/30' : ''} ${app.type === 'rejected' ? 'bg-red-50 dark:bg-red-900/30 text-red-600 border-red-100 dark:border-red-800' : ''}`}>
      ...
      case 'profile':
        return (
          <div className="h-full overflow-y-auto pb-32 bg-[#f4f6f8] dark:bg-[#1C1018] px-6 pt-12 animate-fade-in transition-colors duration-500">
            ...
            <button onClick={() => setIsLoggedIn(false)} className="w-full bg-white dark:bg-gray-800 border border-red-100 dark:border-red-900/50 text-red-500 font-extrabold rounded-2xl py-4 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors shadow-sm">Çıkış Yap</button>
          </div>
        );
              ...
              <MapContainer center={[41.04, 29.0]} zoom={11} className="w-full h-full zoom-control-bottom-right">
                ...
                {backendEvents.map(event => (
                  <Marker 
                    ...
                    icon={getEventIcon(event.category, isDarkMode)} 
        ...
        <>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-gray-800 rounded-t-[2.5rem] z-[1002] p-6 shadow-[0_-20px_40px_rgba(0,0,0,0.15)] animate-slide-up border-t border-gray-100 dark:border-gray-700">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={toggleApplyEvent} className={`flex-[1] flex justify-center items-center font-extrabold text-[11px] px-2 py-4 rounded-2xl transition-all border-2 ${isAppliedToActiveEvent ? 'border-[#68B684] bg-[#68B684]/10 text-[#68B684] shadow-inner' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:border-[#094D92] dark:hover:border-[#68B684]'}`}>
